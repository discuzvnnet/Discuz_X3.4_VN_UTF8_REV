...
var page = {
	converthtml : function() {
		...
		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">Trang trước</a>'+ selector +'<a href="'+ nextpagehref +'">Trang tiếp theo</a>');
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">Tải lại</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Đồng ý" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="Đồng ý" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">Hủy bỏ</a></dd></div>'
			}
...
var formdialog = {
	init : function() {
		$(document).on('click', '.formdialog', function() {
			...
			.error(function() {
				popup.open('Việc gửi biểu mẫu không bình thường và không thể hoàn thành yêu cầu của bạn', 'alert');
			});
			return false;
		});
	}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "Nhận vị trí đã hết thời gian chờ, vui lòng thử lại";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = 'Không thể phát hiện vị trí hiện tại của bạn';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = 'Vui lòng cho phép truy cập vào vị trí hiện tại của bạn';
		        break;
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">Nhấn để làm mới</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('Cuộn xuống để làm mới');
					}
					reloadflag = false;
