...
function previewFormSubmit() {
	...
	if (!selectedTopicId) {
		alert('Vui lòng đẩy các tiêu đề');
		return false;
	}
	if (selectedNormalIds.length < 1) {
		alert('Vui lòng đẩy ít nhất một thông báo vào vùng danh sách');
		return false;
	}
...
function ajaxGetSearchResultThreads() {
	cloudj('#search_result').html('<tr><td colspan="3">Đang tải...</td></tr>');
...
function addMiniportalList(tid) {
	...
	if (selectedNormalIds.length >= 5) {
		alert('Đã có 5 bài viết, vui lòng hủy một số bài viết ở bên phải và thử lại.');
		return false;
	}
...
function removeNormalThreadRecall(displayorder, inNormalEditor) {
	...
	if (threadLi.find('textarea').size()) {
		...
		firstThreadLi.html('<div class="tips">Nhấp vào bên trái<img src="static/image/admincp/cloud/qun_op_list.png" align="absmiddle" />Đẩy thông tin vào danh sách</div>');
		firstThreadLi.show();
	}
}
...
function ajaxUploadQQGroupImage() {
	...
	cloudj('#uploadImageResult').text('Đang tải ảnh lên, vui lòng đợi...');
